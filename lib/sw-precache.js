...
function generateOutputPrecacheConfig(precacheConfig,toReplacePrefix,replacePrefix){
  var strPrecacheConfig=JSON.stringify(precacheConfig)
  console.log(strPrecacheConfig);
  return strPrecacheConfig.replace(new RegExp(toReplacePrefix,"gm"),replacePrefix)
}
...
function generate(params, callback) {
  return new Promise(function(resolve, reject) {
    ...
    console.log(params.stripPrefix,params.replacePrefix)
    console.log(params.assetsPublicPath,params.assetsSubDirectory)
    ...
    var staticPathPrefix=params.assetsPublicPath&&params.assetsSubDirectory&&`${params.assetsPublicPath}${params.assetsSubDirectory}`||null

    var precacheConfig = relativeUrls.sort().map(function(relativeUrl) {
      if(relativeUrl.match(staticPathPrefix)){
        return [`staticPathPrefix+${relativeUrl.replace(relativeUrl.match(staticPathPrefix),'').toString()}`, relativeUrlToHash[relativeUrl]]
      }
    ...
    fs.readFile(params.templateFilePath, 'utf8', function(error, data) {
      ...
      var populatedTemplate = template(data)({
        ...
        precacheConfig: generateOutputPrecacheConfig(precacheConfig,`"staticPathPrefix\\+`,`staticPathPrefix+"`),
